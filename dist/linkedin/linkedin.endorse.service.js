  ...
  try {
    ...
    const endorseRes = await page.evaluate(() => {
      ...
      for (const span of endorseButtons) {
        if (span.innerText.includes("Endorse") && !span.innerText.includes("Endorsed")) {
    ...
    if (!endorseRes) console.error("An error occurred:", LinkedInErrors.NO_SKILLS_FOUND_PENDING_NOT_CONNECTED);
  } catch (error) {}
}
