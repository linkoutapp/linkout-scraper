...
async function comments(page, cdp, data) {
  ...
  try {
    await page.goto(user.endsWith("/") ? user + "recent-activity/comments/" : user + "/recent-activity/comments/");
    ...
    const commentsAndDates = await page.evaluate(() => {
      const commentElements = document.querySelectorAll("div:nth-child(1) > article.comments-comment-item--highlighted");
      ...
      return Array.from(commentElements, commentElement => {
        ...
        const commentNames = document.querySelectorAll(`#${id} .comments-post-meta__name-text`);
        const comments = document.querySelectorAll(`#${id} .comments-comment-item__main-content`);

        const combinedComments = [];
        for (let i = 0; i < Math.min(commentNames.length, comments.length); i++) {
