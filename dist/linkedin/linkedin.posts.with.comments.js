...
async function scrollAndLoad(page) {
  await page.evaluate(async () => {
    ...
    function loadMoreComments() {
      document.querySelector(".comments-comments-list__load-more-comments-button").click();
    }

    function isLoadMorePresent() {
      return document.querySelector(".comments-comments-list__load-more-comments-button") !== null;
    }
  ...
  try {
    ...
    if (repostWithoutCaption) {
      let a = await page.evaluate(() => {
        try {
          const postContentElement = document.querySelector("div > div.feed-shared-update-v2__description-wrapper.mr2 > div > div > span > span");
    ...
    const scrapedData = await page.evaluate(() => {
      const postElement = document.querySelector("div > div.feed-shared-update-v2__description-wrapper.mr2 > div > div > span > span");
      const post = postElement ? postElement.innerText.trim() : "";

      const repostCaptionElement = document.querySelector("[class*=update-content-wrapper] .update-components-text");

      const repostCaption = repostCaptionElement ? repostCaptionElement.innerText : "";
      ...
      const comments = Array.from(document.querySelectorAll(".comments-post-meta__name-text")).map(nameElement => {
        ...
        let comment = "";
        if (commentElement) {
          const commentContentElement = commentElement.querySelector(".comments-comment-item-content-body, .comments-reply-item-content-body");
          if (commentContentElement) {
            const commentContent = commentContentElement.textContent.trim().split("\n");
