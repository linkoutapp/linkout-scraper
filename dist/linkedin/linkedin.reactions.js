...
async function reactions(page, cdp, data) {
  ...
  try {
    await page.goto(user.endsWith("/") ? user + "recent-activity/reactions/" : user + "/recent-activity/reactions/");
    ...
    const reaction = await page.evaluate(() => {
      return Array.from(document.querySelectorAll("div > div.update-components-header.update-components-header--with-control-menu.update-components-header--with-divider.update-components-header--with-image.t-12.t-black--light.t-normal.pt2 > div > span")).map(element => {
        const textContent = element.textContent.trim().split("\n")[0].toLowerCase();
    ...
    const date = await page.evaluate(() => {
      return Array.from(document.querySelectorAll("div > div.update-components-actor.display-flex > div > div > a.app-aware-link.update-components-actor__sub-description-link > span > span:nth-child(1)")).map(element => element.textContent.trim().split("\n")[0].split("•")[0].trim());
    });

    const link = (await page.evaluate(() => window.location.href)) || "";

    const name = await page.evaluate(() => {
      const element = document.querySelector("#recent-activity-top-card > div.break-words.ph5.pv0 > h3");
