...
async function getMessagesList(page) {
  ...
  try {
    await page.waitForFunction(() => {
      const element = document.querySelector(".profile-card-one-to-one__profile-link");
  ...
  const getName = await page.evaluate(() => {
    ...
    const profileLinkElement = document.querySelector(".profile-card-one-to-one__profile-link");
  ...
  const img = (await page.evaluate(n => {
    return (Array.from(document.querySelectorAll("h3")) || []).find(f => f.innerText.trim().indexOf(n) > -1).closest("li").querySelector("img").getAttribute("src");
  }, getName)) || "";
  ...
  const a = {
    ...
    values: await page.evaluate(theName => {
      return (Array.from(document.querySelectorAll(".msg-s-message-list__event")) || []).reduce((all, current) => {
        const topElement = current.querySelector('[data-control-name="view_profile"], .msg-s-message-group__profile-link');
        const top = topElement ? topElement.textContent.trim() : "";

        const timeElement = Array.from(current.querySelectorAll("time")).find(m => m.innerText.includes(":"));
...
async function messagesFromChat(page, cdp, data) {
  ...
  try {
    await page.goto("https://www.linkedin.com/messaging/compose/?connId=" + userName);
    ...
    for (let i = 0; i < Math.max(Math.ceil(count / 20) - 1, 0); i++) {
      await page.evaluate(() => {
        document.querySelector(".msg-s-message-list-content").scrollIntoView({ behavior: "smooth", block: "start" });
      });
